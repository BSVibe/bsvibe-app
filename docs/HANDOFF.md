...
**prod**: `259a7af` (§Ⅸ 이어진 세션 #945 까지) · **열린 PR**: 없음 · **워크트리**: 없음
⚠️ **열린 작업은 이 문서가 아니라 이슈 #928~#939 에 있다.** 아래 §Ⅵ/§Ⅶ 의 "다음 세션 시작점"
목록은 **그 이주 이전에 쓰인 것이라 낡았다** — 착수 전에 이슈를 먼저 보라.
...
## §Ⅸ — 같은 날 이어진 세션 · prod `259a7af`

### ✅ PR #945 — Receive 의 라우팅 키를 라우트가 실제로 심는다 (이슈 #938 닫음)

**⚠️ '두 경로 통합 검토'로 열린 항목인데, 재 보니 통합 이전의 문제였다 — 한쪽이 아예 돈 적이 없다.**

```
trigger_events where trigger_kind='webhook':  13 | 0 | 0
                    (total | has connector_account_id | has resource_id)
```
`receive()` 는 저 두 키가 없으면 **조회 전에** pass-through 로 빠진다. 13건 중 0건 ⇒
바인딩 분기는 프로덕션에서 **한 번도 실행된 적이 없다.**

그리고 한 층 더: 필터는 그 조회 **아래**에 있으므로 `trigger.filters` 도 한 번도 적용된 적이 없다.
이게 중요한 건 **#924 가 `trigger.enabled` 를 지운 근거가 바로 그 문장**이기 때문이다 —
*"Receive 스테이지는 `filters` 만 소비한다"*, *"`filters` 가 이미 '내가 행동하는가'를 표현한다"*.
⇒ **지운 노브를 변호하던 문장이 가리킨 대상이 실은 같은 상태였다.**
(스킬 `a-dead-entity-can-be-the-subject-of-a-live-claim` 의 교과서적 사례.)

두 노브의 차이가 결정을 갈랐다: `enabled` 는 저장 기본값이 `False` 라 배선하면 살아있는 바인딩이
전부 꺼졌을 것이고, **`filters` 는 prod 두 바인딩 모두 `{}`(전부 매치)라 배선해도 동작 변화가 0**이다.
형님이 배선을 택했고, 랜딩 직전 재확인도 통과했다(비어있지 않은 필터가 있으면 중단하도록 걸었다).

* 라우트가 **바인딩이 매치됐을 때만** 두 키를 저장 payload 에 심는다. 미매치면 payload 무변경.
* 키 이름은 `backend/shared/wire_kinds.py` 로 승격 — 양쪽이 한 정의를 import.
  **배선 계약의 철자가 둘이 되는 것이 곧 두 끝이 갈라지는 방식이다.**
* `resource_id` 의 `str()` 캐스팅은 하중(텔레그램 `chat_id` 는 JSON 숫자, 바인딩엔 `"8242700007"`).
* 안 건드린 것: 파서 5종 · `trigger_events.product_id` · 라우트의 product 해석 순서.
* **핵심 테스트는 seam** — 실제 HTTP 라우트로 POST(202) 후 **저장된** 이벤트를 `receive()` 에 먹인다.
  기존 테스트는 전부 양쪽 절반을 **따로** 불렀고, 이 틈이 정확히 그래서 살아남았다.
  절단 5회 중 *"로컬 dict 에 찍고 `event.payload` 를 저장"* 이 5 red 로 잡힌다.

### 📋 이슈 정리 (이번 세션)
* **#932 닫음** — PR #941. ⚠️ 본론은 '한도 추가'가 아니라 **기존 리미터 키가 공격자 손에 있었던 것**.
* **#938 닫음** — 위.
* **#931 에 측정 추가** — 고아 146건: `with_usage` **0** · `total_tokens` **0** · 최신 **2026-08-14** ·
  139/146 은 `run_id` 조차 없음. ⇒ **#928(과금)이 잃을 데이터가 없다**(숫자는 착지 못 한 POST 안에
  있었고 행엔 적힌 적이 없다 — 소급 복구 불가). #926 이후 신규 유입 **0건**.
  정리는 **삭제가 아니라 `failed` 종결** 권장(#926 의 조건부 flip 재사용, prod 대조군 통과 완료).
  데이터 변경이라 형님 판단 대기.

### ⚠️ 이 문서 자체에 일어난 일
세션 도중 **#940 이 `~/Docs` 를 레포로 흡수**했다. 나는 그때까지 `~/Docs/BSVibe_Handoff_Prompt_2026-09-11.md`
에 쓰고 있었고 **그 파일은 이제 없다**. 마커 6개로 대조하니 전부 이 파일에 살아 있어 유실은 없었다.
⇒ **이주는 다른 세션에 전파되지 않는다. 옛 경로에 쓰는 것은 실패하지 않고 아무도 안 읽는 곳에 성공한다.**
백로그도 같이 옮겨졌다(이슈 #928~#939) — PR 번호가 927→941 로 건너뛴 이유가 그것이다.
