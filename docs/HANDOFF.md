...
| | |
|---|---|
| PR **#998** | `_amain` 의 로깅 설정 순서 (#970) — CI ✅ |
| PR **#999** | #965·#966 긴 런 검증 결과 — CI ✅ |
| PR **#1001** | TMPDIR 누수 수정 (#997) — CI ✅ |
| 이슈 **#997** | TMPDIR 누수 (개설) |
| 이슈 **#1000** | codex·opencode MCP 표면 측정 (개설) |
| **#928** 코멘트 | 한 턴이 런 상한의 4.5배 — 실측 |

⚠️ **#998 은 워커 코드다.** 머지 후 `launchctl kickstart -k` 필요.
...
1. **#957 런의 parked Decision `db492740`** — retry 또는 discard. **슬롯 1개를 잡고 있다**
2. **#991 워커 identity 분리** — launchd plist (그대로)
3. **#970 잔여 = 로그 로테이션 하나** — sudo (그대로)
4. **#997 기존 18,810개 삭제** 승인
5. 이월: **#935 시크릿 로테이션 2건** · **#937 리포 밖** · Notion 아카이브 2판
