...
> ✅ **2026-09-17 19:10~19:24 KST 에 걸었다.** prod `8ede21b`.
> 태스크 `6af37139` (런 `143d048c`, #957 구현 작업 · `agentic=t`).

- [x] `agent_loop.act` 가 도는 긴 런(수 분 이상)을 하나 건다
      → **832.6초 = 13분 53초.** prompt 8,919,872 / completion 64,909 토큰.
      claim 10:10:42.702Z → done 10:24:35.327Z
- [x] 그 태스크가 **재전달되지 않는다** — `executor_task_redelivered` 로그 0건
      → 40초 간격 **21회 샘플 내내 0**. `executor_task_redispatched` 도 0
      ⚠️ **양성 대조군**: 같은 스트림·같은 grep 으로 `executor_task_dispatched`
      가 **0 → 2 로 움직였다.** 0 이 "여기엔 없다"가 아니라 "일어나지 않았다"인
      근거는 그것뿐이다 — 안 움직이는 카운터는 아무것도 안 재고 있는 것이다
- [x] 워커에 같은 `task_id` 의 execute 메시지가 **한 번만** 도착한다
      → `bsvibe-worker-mac-mini-e2e.log` 에 정확히 세 줄:
      `task_claimed` 10:10:42.816 · `task_received` 10:10:42.816 ·
      `task_completed` 10:24:35.441. 중복 도착 0
- [x] ⚠️ **#966 체크리스트 3번을 여기서 같이 본다** — 긴 `act` 턴이 안 잘리는지.
      → **안 잘렸다.** 832.6s 를 완주하고 `done`. `agent_loop.act` 예산 3600s 의 23%

  ### ⭐ 이게 왜 이 설계의 근거인가 — 숫자로

  재전달 유예는 **30초**다. lease 방식이었다면 이 턴 하나가 도는 동안
  갱신이 없을 때마다 후보가 됐을 것이고, 832.6 ÷ 30 ≈ **27회**의 중복 실행
  기회가 있었다. 실제 재전달은 **0회**다. 축이 타이머가 아니라
  `claimed_at IS NULL` 이라서, **길이가 아무 역할을 안 한다.**

  ⇒ prompt 8.9M 토큰짜리 턴이 두 번 돌면 LLM 비용이 그대로 두 배가 된다.
  이 PR 이 lease 를 안 고른 이유가 바로 이 칸이고, 이제 prod 에서 봤다.
...
| ~~**긴 `act` 턴이 중복되지 않는다** ⭐~~ | ✅ **2026-09-17 19:10 에 걸었다** — 832.6초 턴, 재전달 0 | 위 ⭐ 절 참조 |
| ~~**#966 체크리스트 3번**(긴 `act` 턴이 안 잘리는지)~~ | ✅ **같은 런에서 봤다** — 안 잘리고 `done` | 위 ⭐ 절 참조 |
