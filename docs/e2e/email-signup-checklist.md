...
> 🚨 **2026-09-18 정정 — 이 문서의 전제가 틀렸다.**
>
> 아래에 *"문은 닫혀 있다 / 이메일 가입이 꺼져 있다"* 로 적힌 것은 **거짓이다.**
> **이메일 가입은 처음부터 켜져 있었다.** GoTrue 에 직접 물어보니
> `429 over_email_send_rate_limit` — **확인 메일을 보내려다** 걸린 것이고,
> 그건 가입을 받아들였다는 뜻이다. `admin@bsvibe.dev` 도 그렇게 만들어졌다.
>
> 왜 틀렸나: PR #1004 의 라우트가 GoTrue 의 **모든** 거절을
> `403 "signup is not available for this instance"` 로 뭉갰다. 그 403 을 보고
> 내가 *"가입이 꺼져 있다"* 로 읽었다 — **내가 만든 진단 문구에 내가 속았다.**
> PR #1009 가 이유별로 갈랐다(`signup_disabled`→403 · 레이트리밋→429 · 4xx→400 ·
> 모르는 이유→502). prod 실측(2026-09-18 19:5x):
>
> | 입력 | 이전 | 지금 |
> |---|---|---|
...
>
> ⇒ **"403 을 봤다"는 가입이 꺼졌다는 증거가 아니다.** 그걸 확인하려면
> `error_code` 를 보거나 GoTrue 에 직접 물어라.
> ⚠️ 그리고 **GitHub OAuth 는 실제로 꺼져 있다**(`provider is not enabled`) —
> 그쪽은 형님이 켜기로 했다. 이메일과 혼동하지 마라.
