...
그중 결정적 요인 하나: **아키텍처 문서가 레포 밖(`~/Docs/`)에 있었다.** 리뷰어는 PR에 없는 문서로 PR을 검증할 수 없다. 그래서 이 문서가 여기 있다.

> **2026-09-12 개정 — 문서는 전부 레포 안으로 들어왔다.** 원래 정책은 *"불변식만 코드 옆, 핸드오프·상태·감사는 `~/Docs/`"* 였다. 그 분리가 **원격 실행(remote/cloud 세션)에서 무너진다**: 원격 에이전트는 clone 하나만 갖고, `~/Docs/` 는 이 맥에만 있는 별개 레포였다. 리뷰어가 PR 밖 문서로 PR 을 검증할 수 없다는 바로 그 이유로, 원격 에이전트도 clone 밖 문서로 일할 수 없다.
> 
> 이제 **현행 컨텍스트는 전부 `docs/` 안에 산다**:
> 
> | 경로 | 내용 |
> |---|---|
...
> | `docs/e2e/` | E2E 체크리스트 |
> 
> 종결된 이력만 `BSVibe/internal-docs` 레포에 남는다. 코드 주석이 그것을 인용할 때는 `internal-docs:<파일명>` 으로 표기한다 — **경로처럼 생긴 문자열을 쓰지 않는다.** 열리지 않는 경로는 열리는 척하기 때문이다(이 정리 직전, 코드가 인용하던 `~/Docs/` 경로 13개 중 **8개가 이미 죽은 링크**였다).
...
*감사 원본: `internal-docs:BSVibe_Reality_Audit_2026-07-14.md` · 진단 스킬: `half-wired-subsystem-audit`*
