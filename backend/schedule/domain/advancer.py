...
v1 shipped two placeholder impls (one-shot + fixed-interval) while the cron
parser was deferred. Real cron-expression evaluation landed in
:class:`CronScheduleAdvancer`, which is now the only impl — the placeholders
had zero callers and were deleted 2026-08-21. The Protocol stays: a different
recurrence algebra still drops in with zero changes elsewhere.
"""
...
class CronScheduleAdvancer:
    ...
    fields, UTC). This is what the production
