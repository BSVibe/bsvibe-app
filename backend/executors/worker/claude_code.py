...
def _unsanctioned_abort(
    ...
    * **Absence** — none (or only some) of BSVibe's tools arrived. An agent with no tools does
    ...
    Neither half is ever relaxed. #692 relaxed the excess half for ``client_attach`` so the
    CLI could act with its own hands on the founder's tree; that exemption is gone — every
...
def _exposed_tools_are_ours(event: dict[str, Any], allowed: list[str]) -> str | None:
    ...
    ONE contract: exposed must be EXACTLY our set. The agent reaches run state only through
    BSVibe's tools; anything else is hands we did not sanction.
