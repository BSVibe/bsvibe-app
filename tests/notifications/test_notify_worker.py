...
async def test_shipped_link_is_rendered_as_report_cta(
    sf: async_sessionmaker[AsyncSession],
) -> None:
    """A ``shipped`` deliverable's trailing line is a 'View report' CTA to the
    deliverable, not the generic Brief-review phrasing."""
    ws = uuid.uuid4()
    await _seed(
        sf,
        ws=ws,
