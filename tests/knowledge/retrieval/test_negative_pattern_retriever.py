...
async def _retract_note(vault_root: Path, *, workspace_id: str) -> None:
...
async def test_retracted_rejection_stops_surfacing(vault_root: Path, workspace_id: str) -> None:
    ...
    **이 리트리버는 안 봤다** — 2시간 뒤 런(``abe9e2b9``)의 판사 기준에 그 문장이
    ...
    await _retract_note(vault_root, workspace_id=workspace_id)

    after = (
        await KnowledgeFactory(workspace_id=workspace_id, vault_root=vault_root)
        .retriever()
