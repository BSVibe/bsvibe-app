...
class TestConceptSynthesisBody:
    ...
    @pytest.mark.asyncio
    async def test_promotion_stamps_localized_display_label_in_non_english_ws(
        self, workspace_storage: FileSystemStorage
    ) -> None:
        """Founder decision 2026-07 — in a non-English workspace, a newly promoted
        concept is stamped with a localized ``display_labels`` entry (generated by
        the routed framer) so its graph node renders in the workspace language.
        The concept id + H1 stay the English identifier."""
        from backend.identity.output_language import set_output_language
        from backend.knowledge.canonicalization.store import NoteStore

        for i in range(2):
            await workspace_storage.write(
                f"garden/seedling/http-client-{i}.md",
                "---\ntags:\n  - settle\n  - http-client\n---\n"
                f"# Http client {i}\n\nUse a pooled client for outbound HTTP.\n",
            )
        ...
        got = await NoteStore(workspace_storage).read_concept("http-client")
        assert got is not None
        assert got.display_labels == {"ko": "HTTP 클라이언트"}
        # Identity unchanged (English display from the deterministic title-caser).
        assert got.concept_id == "http-client"
        assert got.display == "Http-client"

    async def test_promotion_skips_label_in_english_workspace(
        self, workspace_storage: FileSystemStorage
    ) -> None:
        """English workspace (the default) needs no localized label — the English
        display IS the identity, so the framer's ``label`` is never called and no
        ``display_labels`` are written."""
        from backend.knowledge.canonicalization.store import NoteStore

        for i in range(2):
            await workspace_storage.write(
                f"garden/seedling/cookie-jar-{i}.md",
                "---\ntags:\n  - settle\n  - cookie-jar\n---\n"
        ...
        class _Framer:
            ...
            async def label(self, *, concept: str) -> str | None:  # pragma: no cover
                called["label"] = True
                return "should-not-be-used"

        service = await _make_service(workspace_storage, safe_mode=False)
        # default output language is "en"
        await GardenObservationPromoter(service, framer=_Framer()).promote()

        got = await NoteStore(workspace_storage).read_concept("cookie-jar")
        assert got is not None
        assert got.display_labels == {}
        assert called["label"] is False
