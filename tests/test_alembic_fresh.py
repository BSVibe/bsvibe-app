...
2. The expected head revision (``connector_delivery_config``) is stamped
   in ``alembic_version``.
...
def test_fresh_pg_upgrade_round_trip():
    ...
    assert stamped == "connector_delivery_config", (
        f"expected head connector_delivery_config, got {stamped}"
    )
