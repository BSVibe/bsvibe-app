...
2. The expected head revision (``decision_resolve``) is stamped
   in ``alembic_version``.
...
def test_fresh_pg_upgrade_round_trip():
    ...
    assert stamped == "decision_resolve", f"expected head decision_resolve, got {stamped}"
    ...
    _alembic(["upgrade", "head"], env_extra=env_extra)
    stamped = asyncio.run(_stamped_head(url))
    assert stamped == "decision_resolve"
