...
2. The expected head revision (``product_resources``) is stamped
   in ``alembic_version``.
...
def test_fresh_pg_upgrade_round_trip():
    ...
    assert stamped == "product_resources", f"expected head product_resources, got {stamped}"
    ...
    _alembic(["upgrade", "head"], env_extra=env_extra)
    stamped = asyncio.run(_stamped_head(url))
    assert stamped == "product_resources"
