...
2. The expected head revision (``executor_artifact_capture``) is stamped
   in ``alembic_version``.
...
def test_fresh_pg_upgrade_round_trip():
    ...
    assert stamped == "executor_artifact_capture", (
        f"expected head executor_artifact_capture, got {stamped}"
    )
