...
2. The expected head revision (``workspace_audit_retention``) is stamped
   in ``alembic_version``.
...
def test_fresh_pg_upgrade_round_trip():
    ...
    assert stamped == "workspace_audit_retention", (
        f"expected head workspace_audit_retention, got {stamped}"
    )
