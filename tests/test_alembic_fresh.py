...
2. The expected head revision (``bundle_g_glue``) is stamped
   in ``alembic_version``.
...
def test_fresh_pg_upgrade_round_trip():
    ...
    assert stamped == "bundle_g_glue", f"expected head bundle_g_glue, got {stamped}"
