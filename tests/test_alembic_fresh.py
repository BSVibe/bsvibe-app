...
2. The expected head revision (``connector_accounts``) is stamped
   in ``alembic_version``.
...
def test_fresh_pg_upgrade_round_trip():
    ...
    assert stamped == "connector_accounts", f"expected head connector_accounts, got {stamped}"
    ...
    _alembic(["upgrade", "head"], env_extra=env_extra)
    stamped = asyncio.run(_stamped_head(url))
    assert stamped == "connector_accounts"
