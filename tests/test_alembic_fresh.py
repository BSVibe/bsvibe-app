...
2. The expected head revision (``bundle_h_workspaces``) is stamped
   in ``alembic_version``.
...
def test_fresh_pg_upgrade_round_trip():
    ...
    assert stamped == "bundle_h_workspaces", f"expected head bundle_h_workspaces, got {stamped}"
    ...
    _alembic(["upgrade", "head"], env_extra=env_extra)
    stamped = asyncio.run(_stamped_head(url))
    assert stamped == "bundle_h_workspaces"
