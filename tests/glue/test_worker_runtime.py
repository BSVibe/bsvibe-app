...
async def test_production_deps_drive_greenfield_direct_run_to_delivery(
    ...
    async with sf() as s:
        ...
        # gate_expected=False, so the L-I3d ratchet treats it as a legitimate
        # early-stage skip (founder: distinguish "couldn't verify" from
        # "legitimately skipped") and auto-verifies rather than nagging review.
        # (A repo WITH a stack but no gate would be gate_expected=True → review —
        # see the native ratchet tests in test_proved_invariant.)
...
async def test_settle_memory_extractor_factory_returns_worth_remembering(
    sf: async_sessionmaker[AsyncSession],
    workspace_id: uuid.UUID,
    account_id: uuid.UUID,
    kms_key: None,
    ...
    """With ONE active account, the worth-remembering gate factory builds a
    routed extractor whose ``extract`` parses the model's JSON verdict into a
    RememberableKnowledge. The LLM is stubbed at the LlmClient boundary."""
    ...
    _patch_scripted_llm(monkeypatch, _ScriptedCompletion([{"content": verdict}]))

    factory = runtime.build_settle_memory_extractor_factory(
        session_factory=sf, settings=get_settings()
    )
    extractor = await factory(region="us-1", workspace_id=workspace_id)
    assert extractor is not None
    settlement = Settlement(
        ...
        verified=True,
        summary="made the webhook receiver idempotent",
        intent_text="harden webhook handling",
    )
    memory = await extractor.extract(settlement)
    assert memory is not None
    assert memory.topic == "Idempotent webhooks"
    assert "event id" in memory.insight
    get_settings.cache_clear()


async def test_settle_memory_extractor_factory_none_route_writes_nothing(
    sf: async_sessionmaker[AsyncSession],
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    """No routed account → the gate factory returns ``None`` (the sink then
    writes nothing for routine work), and redis is threaded into the resolver."""
    ...
    assert captured["redis"] is sentinel
    get_settings.cache_clear()
