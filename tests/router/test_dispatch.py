"""Tests for backend.router.dispatch — end-to-end orchestration."""
...
from backend.router.budget.models import BudgetEnforcement, BudgetScope
...
class TestErrors:
    ...
    async def test_inactive_account_raises_not_found(
        ...
        accounts = ModelAccountService(session, cipher=cipher)
        ma = await _make_account(accounts, workspace_id, account_id)
        from backend.router.accounts.schemas import ModelAccountUpdate
