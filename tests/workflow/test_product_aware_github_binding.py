...
async def _seed_binding(
    ...
    *,
    ...
    row = ConnectorAccountRow(
        ...
        delivery_config={"repo": repo, "base_branch": "main"},
        is_active=is_active,
    )
...
class TestProductSelectsTheBinding:
    ...
    async def test_no_matching_binding_returns_none(
        self, sf: async_sessionmaker[AsyncSession]
    ) -> None:
        """No binding carries the product's repo → ``None``, never a stranger's.

        ``None`` sends the caller to the product-workspace provisioner and skips
        github delivery — the deliberate safe outcome.
        """
        ws = uuid.uuid4()
        async with sf() as s:
...
class TestProvisionerThreadsTheProduct:
    ...
    async def test_provision_skips_when_no_binding_matches_the_product(
        self, sf: async_sessionmaker[AsyncSession], cipher: CredentialCipher, tmp_path: Path
    ) -> None:
        """No matching binding → no clone at all, so the product-workspace
        provisioner downstream materialises the RIGHT checkout."""
        ws = uuid.uuid4()
        async with sf() as s:
